...
namespace KaiHelper.Tracker
{
    class WayPoint
    {
        ...
        void Drawing_OnDraw(EventArgs args)
        {
            try
            {
                if (!_menu.Item("AWPMap").GetValue<bool>()) return;
                foreach (Obj_AI_Hero hero in ObjectManager.Get<Obj_AI_Hero>().Where(hero => hero.IsEnemy&&hero.IsVisible && !hero.IsDead && hero.IsValid && hero.IsMoving))
                {
                    ...
                    {
                        var beginPoint = waypoints[i].To3D();
                        var endPoint = waypoints[i+1].To3D();
                        ...
                        if (i != lengthPoint)
                        {
                            Drawing.DrawLine(p1Map[0], p1Map[1], p2Map[0], p2Map[1], 2, System.Drawing.Color.White);
                        }
                        else
                        {
                            var r = 25 / p2Map.Distance(p1Map);
                            var enp = new Vector2(r * p1Map.X + (1 - r) * p2Map.X, r * p1Map.Y + (1 - r) * p2Map.Y);
                            Drawing.DrawLine(p1Map[0], p1Map[1], enp[0], enp[1], 2, System.Drawing.Color.White);
                            Utility.DrawCircle(endPoint, 50, System.Drawing.Color.Red);
                            Utility.DrawCircle(endPoint, 50, System.Drawing.Color.FromArgb(50, System.Drawing.Color.Red), -2);
                            Helper.DrawText(_largefont, timer.ToString("F"), (int)p2Map[0], (int)p2Map[1] - 10, Color.White);
                            Helper.DrawText(_largefont, hero.SkinName, (int)p2Map[0], (int)p2Map[1] + 18, Color.White);
                        }
                        if (_menu.Item("AWPMiniMap").GetValue<bool>())
                        {
                            ...
                            if (i == lengthPoint)
                            {
                                Helper.DrawText(_smallfont, hero.SkinName, (int)p2MiMap.X, (int)p2MiMap.Y - 6, Color.Pink);
                            }
                            Drawing.DrawLine(p1MiMap[0], p1MiMap[1], p2MiMap[0], p2MiMap[1], 1, System.Drawing.Color.Yellow);
                        }
                    }
                }
            }
